...
## Интерактивный режим (через Claude Code)

Вместо скрипта можно запустить через Claude Code — он сам спросит параметры и будет сопровождать каждый шаг:
...
Каждый шаг можно запустить независимо:

```
/data-collection    только сбор (нужен domain)
/data-quality       только чистка (нужен путь к parquet)
/annotation         только разметка (нужен путь к parquet)
/active-learning    только AL-отбор (нужен путь к parquet)
```
