...
**ВАЖНО:** После получения подтверждения на старт (`y`) — выполняй все шаги последовательно БЕЗ запросов подтверждения. Не спрашивай "продолжить?", "создать файл?", "перейти к шагу N?" между шагами.

Останавливайся и жди ответа пользователя в трёх случаях:
- **❗ HITL-1** — после авторазметки, когда нужен исправленный файл от человека
- **❗ HITL-2** — перед обучением модели, когда нужно подтверждение датасета
- **❓ Уточнение** — если встретил неоднозначность, без разрешения которой невозможно продолжить корректно (например: конфликт в источниках данных, неизвестный формат файла, критическая ошибка агента). Задавай ровно один конкретный вопрос и жди ответа.

Всё остальное — создание файлов, запуск кода, переходы между шагами, некритичные решения — выполнять автоматически без вопросов.
